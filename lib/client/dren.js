	...
	D.init = function (t) {
		...
		if(!T.isNA(D.template_scripts)){
			...
			for(var i in templatesUnInited){
				...
				var scriptsRequired = D.template_scripts[i];
				if(scriptsRequired){
					if(scriptsRequired.constructor.name !== "Array"){ // Force format [[string]]
					...
					console.log("Requiring scripts successions: "+JSON.stringify(scriptsRequired));

					k = 0; // Reset counter
					function loopRequire(){
						console.log(k, scriptsRequired[k])
						requirejs(scriptsRequired[k], function(){
							k++;
							if(scriptsRequired[k]){
								return loopRequire();
							} else {
								...
								for(var j = 0, J = arguments.length; j < J; j++){
									...
									if(typeof templateObject == "function"){
										...
										console.info("Script loader not catched", templateObject);
									}
								}
							}
						});
